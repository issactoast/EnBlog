...
GPS is probably the most popular data format, which has information about the position of the device. At the beginning of my research, I tried to use it to record my commute route. To record the telematics data, I attached my phone to my vehicle and recorded the position of my vehicle per second using a smartphone sensor app.
...
Thus, since we do not need redundant information, let us select the data for every 25 data points. Also, there is no negative speed, so let us replace them as zero value.

```{r}
gpsdata <- gpsdata[seq(1, length(gpsdata$Timestamp), by = 25),]
gpsdata$speed[gpsdata$speed < 0] <- 0
```
...
<a href="https://theissaclee.com/post/stat2010/" target="_self">Return to the telematics project page</a>
